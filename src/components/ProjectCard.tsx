...
export default function ProjectCard({id, title, description, stack, imageUrl, projectUrl}: {id: number, title: string, description: string, stack: string[], imageUrl: string, projectUrl: string}) {
    return (
        <div className={`lg:w-6xl lg:h-[480px] md:w-[784px] md:h-[360px] w-xl h-[772px] shadow-lg rounded-lg flex ${id % 2 !== 0 ? "lg:flex-row md:flex-row flex-col" : "lg:flex-row-reverse md:flex-row-reverse flex-col-reverse"}`}>
            <div className="bg-gray-50 w-xl h-[480px] p-12">
                <img src={imageUrl} alt="Fiskil Project Mockup" className="w-full h-full object-cover rounded-lg" />
            </div>
            <div className=" w-xl h-[480px] p-12 flex flex-col justify-start items-start gap-6">
